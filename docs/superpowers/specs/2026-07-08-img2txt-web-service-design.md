...
`CorrectionBackend` 인터페이스: `correct_batch(paragraphs: list[str], model: str) -> list[str]`. 구현체 3종:
...
잡 메타데이터(status/files/options/summary)는 `JobStore`(메모리 dict). 서버 재시작 시 진행 상태 소멸(최소 범위라 허용), 디스크 산출물은 남아 다운로드 가능. 자동 정리는 생략(수동).
...
- `POST /api/jobs` (multipart: files[], correct, model, backend): 업로드 저장 -> 잡 생성(queued) -> 백그라운드 제출 -> `{id}`(201). 파일 0개 400.
- `GET /api/jobs/:id`: 스토어의 `Job`(+phase, correction 진행률). 미존재 404.
- `POST /api/jobs/:id/retry/:fileId`: 실패 페이지 재-OCR.
...
- 신규 핵심:
  ...
  3. `JobStore`: 상태 전이(queued→processing→done|failed), 재시도.
  4. API: FastAPI `TestClient`로 엔드포인트 계약(생성->폴링->다운로드), 파이프라인 모킹(실제 OCR/CLI 없이).
...
- 잡 상태는 메모리라 서버 재시작 시 진행 중 잡 소멸(최소 범위 허용).
