...
- 결정사항: **전처리 레버는 일괄 적용하지 않음(잠정)** — deskew는 문서 품질에 따라 개선/악화가 뒤집혔고(최저 품질에서 +0.71%p 악화), contrast/upscale은 기각. baseline 유지. 절대 CER(17~53%)은 라벨 정렬 한계로 과대추정임을 원본 대조로 확인.
- 남은 일: (1) 스펙 7절 LLM 보정 모델 비교 플랜 (다음 트랙, 새 세션 권장). (2) 라벨 정렬 한계 완화(순서 무관 보조 지표) 검토. (3) 책 도메인 소량 정답셋 확보 시 재검증.
