...
## 2026-07-20 - LLM 보정 가성비 실측 착수 시도 (codex 크레딧 블로커로 보류)
- 상태: 부분 완료 (codex 워크스페이스 크레딧 소진으로 실측 0건, 이슈로 위임)
- 요약: 스펙 7절 LLM 보정 가성비 실측 착수 준비. 환경 조사 완료(AI Hub 데이터 jpg 11,734장 + 라벨 json 11,734개, codex CLI 0.144.5, ollama 로컬모델 다수). 사용자가 "클라우드 codex 포함" 경로 결정 → AI Hub 데이터 외부 전송 리스크 감수(#12 게이트 해소). 앵커 gpt-5.5 더미 실호출이 "out of credits"로 실패 → 크레딧 충전 전까지 실측 불가. 코드 변경 없이 조사만 수행.
- 커밋/PR: 없음 (코드 변경 0, 조사만)
- 남은 일: codex 크레딧 충전 → 7.1 리팩터(`cli.py:58` gpt-5.5 하드코딩 → 모델 파라미터화) → 모델 식별자-비용 출력 검증 → 앵커+저비용 후보 실측 → 7.3 채택 판정. 재개는 이슈 #15.
- 관련 문서: docs/superpowers/specs/2026-07-13-ocr-llm-quality-harness-design.md 7-8절, 이슈 #15(중요)/#16(백로그)
- 상세 히스토리: 없음
